 ...
 *         During the expansion, the following rules appy:
 * 
 *         The propagation is based on the following parameters: - node's name and
 *         node's attributes - fragments belonging to the subtree
 * 
 *         Logic: - it two nodes are equal - if the have the same fragments
 *         => EQUAL - if they don't have the same fragments => EQUAL + MOVE of
 *         non common fragments
 * 
...
public class Propagation extends Phase {
	...
	public void compute() throws ComputePhaseException {
		...
		try {
			// Prendo i parametri della fase dal file di configurazione
	...
	private void propagationToParent(int indexA, int indexB) {
		...
		if ((indexA > 0) && (indexB > 0)) {
			...
			if ((nodeA.inRel == Relation.NO) && (nodeB.inRel == Relation.NO)
				...
				// Se i frammenti nn sono identici, eguaglio i nodi spostando
				// gli opportuni frammenti
				else if (forcematch) {
					tmpDebug += " frammenti:no move:";
